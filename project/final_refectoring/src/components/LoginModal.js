...
function LoginModal(props){
  ...
  const {values, handleChange, handleLogin } = UseForm({
          ...
          <form>
            {
              ...
              ?(
                <>
                <input type="text" placeholder="아이디를 입력하세요" name="username" value={values.username} onChange={handleChange}/>
                <input type="password" placeholder="비밀번호를 입력하세요" name="password" value={values.password} onChange={handleChange}/>
                <button className="JoinLoign-button" onClick={handleLogin}>{joinLoign}</button>
                </>
              )
              :(
                <>
