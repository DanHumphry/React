...
import UseForm from '../container/UseForm'
  ...
  //필요한거 정리 
  // 1. input onChange, onClick 로그인, 회원가입,

function LoginModal(){
  let [joinLoign,setJoinLogin] = useState('로그인');

  const { handleNameChange, handlePasswordChange, LoginSubmit, joinSubmit, history} = UseForm();
          ...
          <form>
            {
              ...
              ?(
                <>
                <input type="text" placeholder="아이디를 입력하세요" name="username" onChange={(e)=>{handleNameChange(e)}}/>
                <input type="password" placeholder="비밀번호를 입력하세요" name="password" onChange={(e)=>{handlePasswordChange(e)}}/>
                <button className="JoinLoign-button" onClick={(e)=>{LoginSubmit(e)}}>{joinLoign}</button>
                </>
              )
              :(
                <>
                <input type="text" placeholder="아이디를 입력하세요" name="username" onChange={(e)=>{handleNameChange(e)}}/>
                <input type="password" placeholder="비밀번호를 입력하세요" name="password" onChange={(e)=>{handlePasswordChange(e)}}/>
                <button className="JoinLoign-button" onClick={(e)=>{joinSubmit(e)}}>{joinLoign}</button>
                </>
              )
            }
          </form>
